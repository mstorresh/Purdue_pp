...
double ffur(double *x, double *par){
    ...
    double re2 = k1 + g1(x,k2,M_PI) + g1(x, k2, -M_PI) + g1(x, k3, 0);
    return re2;
...
void mediumrati(){
    ...
    /* double p1 = 107073000;  //229814000; */
    /* double p2 = 94346000 ;  //458098000; */
    /* double p5 = 9493190;    //161676000; */
    ...
    h1x->Scale(1./(h1x->GetXaxis()->GetBinWidth(1)*p1));
    h2x->Scale(1./(h2x->GetXaxis()->GetBinWidth(1)*p1));
    h1y->Scale(1./(h1y->GetYaxis()->GetBinWidth(1)*p1));
    h2y->Scale(1./(h2y->GetYaxis()->GetBinWidth(1)*p1));
    ...
    h1_sx->Scale(1./(h1x->GetXaxis()->GetBinWidth(1)*p2));
    h1_lx->Scale(1./(h2x->GetXaxis()->GetBinWidth(1)*p2));
    h1_sy->Scale(1./(h1y->GetYaxis()->GetBinWidth(1)*p2));  //quantity for events
    h1_ly->Scale(1./(h2y->GetYaxis()->GetBinWidth(1)*p2));  //also b have to be with pedestal
    ...
    h5_sx->Scale(1./(h5_sx->GetXaxis()->GetBinWidth(1)*p5));
    h5_lx->Scale(1./(h5_lx->GetXaxis()->GetBinWidth(1)*p5));
    h5_sy->Scale(1./(h5_sy->GetYaxis()->GetBinWidth(1)*p5));
    h5_ly->Scale(1./(h5_ly->GetYaxis()->GetBinWidth(1)*p5));
   ...
   //------ new plots ---------------------------------------------------------------------------------------------------------------
    ...
    double param1[npar] = {0.00073, 0.00003, 1.0};
    ...
    std::cout<<"Chisquare low : "<<chi1<<"  Chi/NDF :  "<<chi1/ndf1<<'\n';
         // ----------- medium -------------------------------------------------
    double param2[npar2] = {0.007, 0.0001, 1.0, 0.009, 0.0001};

    TF1 *f21 = new TF1("f21", ffur, -M_PI, M_PI, npar); 
    ...
    std::cout<<"Chisquare medium: "<<chi2<<"  Chi/NDF :  "<<chi2/ndf2<<'\n';
        ...
        //----------- high -------------------------------------------------------
    double param3[npar2] = {0.0185, 0.0002, 1.0, 0.009, 0.001};

    TF1 *f22 = new TF1("f22", ffur, -M_PI, M_PI, npar); 
    ...
    for(int j=1; j<=bin1; j++){
        ...
        sub1 = h1_sy->GetBinContent(j) - ped1;
        sub2 = h1y->GetBinContent(j) -ped2;
        sub3 = h5_sy->GetBinContent(j) - ped3;
    ...
    leg3->AddEntry(h1_larped, "low mul", "l");
    leg3->AddEntry(h2_larped, "medium mul", "l");
    leg3->AddEntry(h5_larped, "high mul", "l");
    leg3->SetTextSize(0.05);
    leg3->Draw();

   
   // --------- labels size ------
    h5_larped->GetXaxis()->SetTitleSize(0.05);
    h5_larped->GetXaxis()->SetTitleOffset(0.95);
    h5_larped->GetXaxis()->SetLabelSize(0.05);

    h5_larped->GetYaxis()->SetTitleSize(0.05);
    h5_larped->GetYaxis()->SetTitleOffset(1.5);
    h5_larped->GetYaxis()->SetLabelSize(0.05);
