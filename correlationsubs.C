...
void correlationsubs(){
   ...
   //------ new plots ---------------------------------------------------------------------------------------------------------------
    ...
    for(int j=1; j<=bin1; j++){
        ...
        sub1 = h3_sho->GetBinContent(j) - ped1;
        sub2 = h3->GetBinContent(j) -ped2;
        sub3 = h3_lar->GetBinContent(j) - ped3;
        h1_larped->SetBinContent(j, sub1);
        h1_larped->SetBinError(j, h3_sho->GetBinError(j));
        h2_larped->SetBinContent(j, sub2);
        h2_larped->SetBinError(j, h3->GetBinError(j));
        h5_larped->SetBinContent(j, sub3);
        h5_larped->SetBinError(j, h3_lar->GetBinError(j));
    }
    ...
    gPad->SetTicks();
    gStyle->SetOptTitle(0);
    h5_larped->GetYaxis()->SetTitle("#frac{dN_{SS}}{d #Delta #Phi_{|#Delta #eta<0.5}}/#frac{dN_{SS}}{d #Delta #Phi_{|#Delta #eta>1.5}}");
